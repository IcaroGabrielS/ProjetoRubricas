...
def import_employees():
    ...
    try:
        ...
        db_transaction = db.session.begin_nested()  # Cria um savepoint
        ...
        # Log de conclusão
        current_app.logger.info(
            f"Importação concluída: {employees_added} adicionados, {employees_updated} atualizados, {len(errors)} erros"
        )
...
def import_companies():
    ...
    try:
        ...
        # Log de conclusão
        current_app.logger.info(
            f"Importação de empresas concluída: {companies_added} adicionadas, {companies_updated} atualizadas, {len(errors)} erros"
        )
...
def associate_employees_by_code():
    ...
    try:
        ...
        for codi_emp, company_id in mappings.items():
            try:
                if not is_valid_uuid(company_id):
                    results["errors"].append(f"ID de empresa inválido para código {codi_emp}")
                    continue
